...
export function capcomBrief(): string {
  return [
    ...
    '- `set_budget` — a ceiling on one agent, a squad (shared) or a mission (shared). The unit'
      + ' is TOKENS (`budget_tokens`: input + output + cache writes, the agent\'s own AND every'
      + ' Task subagent it launches), because the operator runs on a flat plan and quota is'
      + ' what runs out. There is no ceiling in dollars: this fleet is not charged per call,'
      + ' so a dollar figure measures nothing. `budget_min` counts minutes seen WORKING, never'
      + ' wall clock since launch. `spawn_agent` and `launch_squad` take the same, plus'
      + ' `squad_budget_tokens` / `squad_budget_min` for the whole squad.'
