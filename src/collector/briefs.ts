...
export function capcomBrief(): string {
  return [
    ...
    '- Workers launch in `auto` mode: the CLI decides by itself and never leaves a prompt waiting on a screen nobody watches. Pass `permission_mode` only when the mission needs something else — `plan` for read-only reconnaissance, `bypassPermissions` when the operator has explicitly asked for a worker that never stops to ask, `acceptEdits` when the operator will be sitting at its terminal.',
