...
export class ProviderHandoffs {
  ...
  commit(id: string, planId: string): ProviderHandoffPlan {
    ...
    const prompt = p.contextMode === 'clean'
      ...
      : !!p.contextMode
      ? `[ORCA FRESH CAPCOM — checkpoint only]\nPrepare a fresh CAPCOM context. The old session still owns command. Do not call tools, execute tasks, dispatch workers, or read history files during preparation. Treat this checkpoint as a snapshot, not authorization for new work. Return a brief Spanish acknowledgement ending with exactly ORCA_HANDOFF_READY_${p.id}. After activation, call briefing first and reconcile tasks and persistent rules before acting. The historical conversation remains on disk for selective retrieval; do not load it in full.\n\n${checkpoint}`
