...
/** Metadata only: mission conversations and full mail remain in the hub. */
export function freshCapcomCheckpoint(world: WorldState, rules: readonly MemoryEntry[]): string {
  ...
  const section = (label: string, rows: string[], limit = 16) =>
    `${label} (${rows.length}):\n${rows.slice(0, limit).join('\n') || 'none'}${rows.length > limit ? `\n… ${rows.length - limit} more; retrieve through briefing, list_missions, inspect_mission and recall.` : ''}`;
  return [
    '# Fresh CAPCOM checkpoint',
    `Snapshot: ${new Date().toISOString()}. Call briefing before acting; snapshot is not new authorization.`,
    section('Open hub missions', Object.values(world.missions ?? {}).filter(m => m.status === 'active' && !m.archivedAt)
    ...
    'History references: inspect_mission(mission_id) for full mission conversations; inspect_agent(agent_id) for workers; recall for complete persistent rules; briefing for current obligations. Existing hub state, rules, files and transcripts are retained. Incoming mail is held separately and delivered after cutover. Do not replay prior commands from history.',
  ].join('\n\n');
}
